...
def test_pixel_values(image, roi, color_space, expected_values, tolerance=0):
    ...
    if isinstance(expected_values, tuple):
        ...
        mask_percentage = np.count_nonzero(mask == 255) / (mask.shape[0] * mask.shape[1]) * 100

        print_info(f"Percentage of pixels meeting the criteria: {mask_percentage:.2f}%")
        time.sleep(2)
        print_info(f"Average pixel value in the region: {np.mean(region, axis=(0, 1))}")
        time.sleep(2)

        test_result = round(mask_percentage, 2) >= 99.99
        return test_result, mask_percentage
    else:
...
def main():
    ...
    roi = (962, 411, 1680, 1050)  # Define the region of interest (ROI)
    time.sleep(2)
    print_info(f"Testing pixel values in the ROI: {roi}")
    time.sleep(2)
    # Show the captured image
    print_info("Analysing the captured image...")
    x, y, w, h = roi
    ...
    time.sleep(2)
    expected_values = (94, 60, 108)  # Define the expected RGB values
    tolerance = 5  # Define the tolerance (optional)
    test_result, percentage = test_pixel_values(screenshot_np, roi, color_space, expected_values, tolerance)
    time.sleep(2)
